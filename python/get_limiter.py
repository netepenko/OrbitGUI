...
def draw_pos(rad, phi, *args, **kwargs):
    ...
    for i, r in enumerate(rad[:-1]):
        if r == rad[i+1] :
            ...
            y = np.array([y1,y2])
        if i>0 and i<4:
            pl.plot(x,y,color = 'C1', *args, **kwargs)
        else:
            pl.plot(x,y, color = 'C0', *args,**kwargs)
    # all done
...
#        self.r_regions = np.array(r_regions)
    ...
    def draw_side(self, ireg = 0, adjust_aspect = True, *args, **kwargs):
        ...
        for i in range(pol):
            if len(self.polygons[i][0])>10:
                pl.plot(self.polygons[i][0], self.polygons[i][1], color = 'C0', *args, **kwargs)
                continue
            if len(self.polygons[i][0])>5:
                pl.plot(self.polygons[i][0], self.polygons[i][1], color = 'C1', *args, **kwargs)
                continue
            pl.plot(self.polygons[i][0], self.polygons[i][1], color = 'C3',  *args, **kwargs)    
        if adjust_aspect:
