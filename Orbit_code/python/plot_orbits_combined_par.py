...
#
# this uses the information in the dynamic_input file to assign detector id's and colors them appropriately.
...
#import pickle needed to import neutron profile as a background
import numpy as np
import LT.box as B
...
#    n_det = pf.get_value('N_det')
    ...
    dfd = B.get_file(dynamic_file).par
    
    # get channel/detector assignement
    dets = dfd.get_value('detectors', var_type= int)
    try:
        dn = np.array(dfd.get_value('detector_number').split(','), dtype = int)
    except:
        dn = int(dfd.get_value('detector_number'))
...
#    return
    ...
    PD_channel_number = []
    PD_step = []
    for i in range(dets): #,i_d in enumerate(idet):
        cc = i
        idd = i+1
        name_patt = orbit_output_dir + '/track_{}*.data'.format(idd)
        #print name_patt
    ...
    # draw side view
    draw_top_view=True #False
    if draw_top_view:
        f1 = pl.figure(figsize= (11,6))
#        f1=pickle.load(file('neutron.pickle'))
    else:
