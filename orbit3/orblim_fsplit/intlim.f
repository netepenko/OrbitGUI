...
c************************************************************
      ...
      dimension rdet(3), rp(3), ptd(3), du(3), cross(3)
...
c!!!!!! routine to check if within the detector acceptance
c       cp = cos(port)
c       sp = sin(port)
c       cpph = cos(portph)
c       spph = sin(portph)
c       
cc       detector position in cartesian coordinates
c       rdet(1) = RD*cos(phd)
c       rdet(2) = RD*sin(phd)
c       rdet(3) = ZD
c
cc       particle position in cart coords
c       rp(1) = r(1)*cos(r(2))
c       rp(2) = r(1)*sin(r(2))
c       rp(3) = r(3)
c       
cc       vector from detector to particle
c       ptd(1) = rp(1) - rdet(1)
c       ptd(2) = rp(2) - rdet(2)
c       ptd(3) = rp(3) - rdet(3)
c       
cc       detector orientation unit vector
c       du(1) = -cpph*sp
c       du(2) = spph
c       du(3) = cpph*cp
c       
cc       cross product of du and ptd
c       cross(1) = ptd(2)*du(3) - ptd(3)*du(2)
c       cross(2) = -ptd(1)*du(3) + ptd(3)*du(1)
c       cross(3) = ptd(1)*du(2) - ptd(2)*du(1)
c       
cc       distance from detector unit vector to particle
c       d = sqrt(cross(1)**2 + cross(2)**2 + cross(3)**2)
cc       distance from detector to particle along the detector vector
c       dl = ptd(1)*du(1) + ptd(2)*du(2) + ptd(3)*du(3)
c       
c       if (dl .lt. 0.03) then
c           hitlim = .true.
c       endif
