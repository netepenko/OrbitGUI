...
from fpdatafile import fpdfile
...
class view:
    def __init__(self, file, neutron_camera = False):
        ...
        self.d = fpdfile(file)
        # get the data
        self.rt =  self.d.get_data('r') 
        self.xt =  self.d.get_data('x')
        self.yt =  self.d.get_data('y')
        self.zt =  self.d.get_data('z')
        self.psi =  self.d.get_data('psi')
        self.sdlcs =  self.d.get_data('Sdlcs')
        self.Emt =  self.d.get_data('Em')
        # one should make sure that psi has a valid value
        # this should be done in orbit not here
        # in_psi_zero = np.where(self.psi == 0.)
        self.psirel = self.d.get_data('psirel')
