...
c                 to be precise one should convert the xs/ys into toloridal coord to take the shift in the collimator
...
c                 rotation angle should be changed to phd of probe (currently 79.3 in expressions below) 
                  rt_br(1) = xs*cp + ys*sp*spph - zs*sp*cpph
                  rt_br(2) = ys*cpph + zs*spph
                  rt_br(3) = xs*sp - ys*cp*spph + zs*cp*cpph
                  rt(1) = rt_br(1)*cos(PI*79.3/180.0) - rt_br(2)*sin(PI*79.3/180.0) + rt0(1)
                  rt(2) = rt_br(1)*sin(PI*79.3/180.0) + rt_br(2)*cos(PI*79.3/180.0) + rt0(2) 
                  rt(3) = rt_br(3) + rt0(3)
...
c     jdL det.  y-index
                 ...
                 write(orbit_id, '(i7)')  icounter*10000 + 1000*i + 100*j + 10*id + jd
